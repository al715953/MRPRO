...
def initialize_data_layer(ui):
    ...
    try:
        ...
        ui.console.print("[yellow]⚠️ Base de datos no encontrada. Iniciando descarga...[/]")
    ...
    try:
        ...
        if dias_desde_ultimo > 3:
            ui.console.print(f"[cyan]🌐 Sincronizando nuevos sorteos (Último: {ultima_fecha})...[/]")
            if scraper.actualizar_csv():
                history = loader.load_data()
    except Exception as e:
        ui.console.print(f"[dim red]Aviso: No se pudo verificar caducidad de datos ({e})[/]")
...
def main():
    ...
    if not history:
        ui.console.print("[bold red]❌ ERROR CRÍTICO:[/] No se pudo establecer conexión con el histórico.")
        return
